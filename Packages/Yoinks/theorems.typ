  ...
  let boxfmt(name, number, body, title: auto, ..blockargs_individual) = {
    ...
    pad(
      ...
      block(
        ...
        inset: 1.2em,
        radius: 0.3em,
        breakable: tog, // Default is false however we do have som long examples
        ..blockargs.named(),
...
#let proof = (body, of: none) => {
  let startText = if type(of) == label {
    [Proof of #ref(of).]
  } else if of != none {
    [Proof of #of.]
  } else {
  ...
  [#text(style: "italic", startText)\ #pad(left:1.5em)[#body] #align(right)[$square.big$]]
}

#let case = (body, of: "*") => pad(left:1.5em)[
  $bold(#of)$: #body
]